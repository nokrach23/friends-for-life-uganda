(() => {
  ...
  document.querySelectorAll('[data-validated-form]').forEach((form) => {
    ...
    const showError = (field) => {
      ...
      error.textContent = field.validity.valid ? '' : (messages[key] || 'Please check this field.');
      field.setAttribute('aria-invalid', String(!field.validity.valid));
      if (!field.validity.valid) field.setAttribute('aria-describedby', `${field.id}-error`);
  ...
  if (requestedType && typeField) {
    const option = [...typeField.options].find((item) => item.value.toLowerCase() === requestedType.toLowerCase());
